-- ============================================================
-- Thiro.pedido — Simplificar abertura de comanda "delivery"
--
-- Antes, delivery exigia nome, telefone e endereço do cliente pra
-- poder abrir a comanda (regra travada por um CHECK constraint). A
-- gestão pediu pra tirar esses campos e deixar só a taxa de entrega
-- (o pedido em si já vem de fora do sistema — iFood, WhatsApp etc.).
--
...
alter table public.comandas add constraint comandas_dados_por_tipo check (
  ...
  or
  (tipo = 'delivery' and mesa_numero is null and comanda_fisica_id is null)
);
